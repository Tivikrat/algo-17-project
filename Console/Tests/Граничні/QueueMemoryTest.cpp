...
int main() {
    ...
    for (int j = 1; ; ++j)
    {
        ...
        std::cout << j * 10000 << " elements in queue with " << pmc.PagefileUsage << " memory bytes used ("
            << (pmc.PagefileUsage >> 30) << "GB, "
            << ((pmc.PagefileUsage >> 20) % 1024) << "MB, "
            << ((pmc.PagefileUsage >> 10) % 1024) << "KB)\n";
    }
