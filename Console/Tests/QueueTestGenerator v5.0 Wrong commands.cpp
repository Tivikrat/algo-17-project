...
int main() {
    ...
    std::cout << "Enter wrong part percentage (from 0 to 1): ";
    std::cin >> wrongPart;
    ...
    {
        double value = (double)generator() / (double)max;
        if (value < pushPart)
        {
            push();
        }
        else if (value < pushPart + wrongPart)
        {
            output << random_string(rand() % longCount + 1) << '\n';
        }
        else if (size > 0)
        {
            pop();
